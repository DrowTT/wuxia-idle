...
import {
  activeRoundPassiveValue,
  clamp,
  createEffects,
  getHitChance,
  grantNextAttackGuaranteedDodge,
  grantNextAttackGuaranteedHit,
  hasActiveRoundPassive,
  isSupportedPlayerPassive,
  legacyStatsFromPower,
  passiveRounds,
  passiveValue,
  randomFactor,
  randomValue,
...
function normalizeEncounterEnemy(
  enemy: MainStageEnemy,
  index: number,
): EncounterEnemy {
...
export function createEncounter({
  ...
  const resolvedPlayerStats = normalizeCombatStats(
    playerStats ?? legacyStatsFromPower(1_250),
  );
    ...
    : [
        {
          ...
          stats: normalizeCombatStats(
            enemyStats ?? legacyStatsFromPower(enemyPower || 900),
          ),
        },
      ];
  ...
  const passiveEffects = playerPassives
    .filter(isSupportedPlayerPassive)
    .map((effect) => ({ ...effect }));
  const startingRage = passiveEffects.reduce(
    (total, effect) =>
      total + (effect.kind === "battle-start-rage" ? passiveValue(effect) : 0),
    0,
  );
  const startingDodge = passiveEffects.reduce(
    (total, effect) =>
      total +
      (effect.kind === "battle-start-dodge"
        ? Math.floor(passiveValue(effect))
        : 0),
    0,
  );
  const enemyActionBlockedUntilRound = passiveEffects.reduce(
    (latest, effect) =>
      effect.kind === "block-enemy-actions-for-rounds"
        ? Math.max(latest, passiveRounds(effect))
        : latest,
    0,
  );
  return {
    ...
    dungeonMechanic,
    enemyName:
      enemyName ?? resolvedEnemies.map((enemy) => enemy.name).join("、"),
    enemyPower: resolvedEnemies.reduce(
      (total, enemy) => total + getCombatPower(enemy.stats),
      0,
    ),
    ...
    playerLethalGuardCharges: passiveEffects
      .filter((effect) => effect.kind === "survive-lethal")
      .reduce((total, effect) => total + Math.floor(passiveValue(effect)), 0),
    enemyActionBlockedUntilRound,
    playerOuterSkills: playerOuterSkills.map((skill) => ({ ...skill })),
    nextOuterSkillIndex: 0,
    round: 0,
    maxRounds:
      COMBAT_BALANCE.baseMaxRounds +
      Math.max(0, resolvedEnemies.length - 1) *
        COMBAT_BALANCE.additionalRoundsPerEnemy,
...
function statsFor(encounter: Encounter, combatant: CombatantRef): CombatStats {
  return combatant.side === "player"
    ...
    : encounter.enemies[combatant.enemyIndex]!.stats;
}
...
function effectsFor(
  ...
  return combatant.side === "player"
    ? encounter.playerEffects
    : encounter.enemies[combatant.enemyIndex]!.effects;
}

function hpFor(encounter: Encounter, combatant: CombatantRef): number {
  return combatant.side === "player"
    ? encounter.playerHp
    : encounter.enemies[combatant.enemyIndex]!.hp;
}

function maxHpFor(encounter: Encounter, combatant: CombatantRef): number {
  return combatant.side === "player"
    ? encounter.playerMaxHealth
    : encounter.enemies[combatant.enemyIndex]!.maxHealth;
}
...
function applyDamage(
  ...
  setHp(
    encounter,
    combatant,
    hpFor(encounter, combatant) - Math.max(0, amount),
  );
  if (
    combatant.side === "player" &&
    encounter.playerHp <= 0 &&
    encounter.playerLethalGuardCharges > 0
  ) {
...
function displayName(encounter: Encounter, combatant: CombatantRef): string {
  return combatant.side === "player"
    ? "你"
    : encounter.enemies[combatant.enemyIndex]!.name;
}
...
function toTurnAction(
  ...
  return combatant.side === "player"
    ...
    : {
        side: "enemy",
        enemyId: encounter.enemies[combatant.enemyIndex]!.id,
        isCounter,
        ...(isCombo ? { isCombo: true } : {}),
      };
}

function toCombatantRef(
  encounter: Encounter,
  action: EncounterTurnAction,
): CombatantRef | null {
  if (action.side === "player") return { side: "player" };
  const enemyIndex = encounter.enemies.findIndex(
    (enemy) => enemy.id === action.enemyId,
  );
  return enemyIndex >= 0 ? { side: "enemy", enemyIndex } : null;
}

function counterTurnAction(
  encounter: Encounter,
  counterAttacker: CombatantRef,
  originalAttacker: CombatantRef,
): EncounterTurnAction {
...
function targetForTurnAction(
  ...
  if (action.targetEnemyId) {
    const enemyIndex = encounter.enemies.findIndex(
      (enemy) => enemy.id === action.targetEnemyId && enemy.hp > 0,
    );
...
function resolveHit(
  ...
  attackerEffects: CombatEffectState,
  defenderEffects: CombatEffectState,
  random: () => number,
  guaranteedHit = false,
): { hit: boolean; guaranteed: boolean } {
  ...
  return {
    hit: randomValue(random) * 100 < getHitChance(attacker, defender),
    guaranteed: false,
  };
}
...
function performAttack(
  ...
  const rageBeforeSkill =
    skill &&
    attackerRef.side === "player" &&
    !isCounter &&
    encounter.playerRage >= 100
      ? encounter.playerRage
      : 0;
  ...
  if (
    attackerRef.side === "enemy" &&
    encounter.round <= encounter.enemyActionBlockedUntilRound
  ) {
  ...
  const weaponAffinityDamageMultiplier = weaponAffinityActive
    ? COMBAT_BALANCE.weaponAffinityDamageMultiplier
    : 1;
  const weaponAffinityEffectMultiplier = weaponAffinityActive
    ? COMBAT_BALANCE.weaponAffinityEffectMultiplier
    : 1;
  ...
  if (skillAction) {
    ...
    encounter.logs.push(
      `${attackerName}施展${skillAction.name}，怒气倍率 x${skillAction.multiplier.toFixed(2)}。`,
    );
    if (skillAction.weaponAffinityActive)
      encounter.logs.push("所持兵器与外功相契，招式威力更盛。");
  }
  ...
  const hit = resolveHit(
    ...
    defenderEffects,
    random,
    Boolean(activeSkill?.guaranteedHit),
  );
  if (!hit.hit) {
    encounter.logs.push(
      hit.guaranteed
        ? `${defenderName}身形一晃，避开了${attackerName}的必中一击。`
        : `${attackerName}一招落空，被${defenderName}闪开。`,
    );
    return {
      action: createCombatAction(
        encounter,
  ...
  if (
    defenderRef.side === "player" &&
    hasActiveRoundPassive(encounter, "damage-immunity-for-rounds")
  ) {
    encounter.logs.push(
      `${defenderName}护体无伤，化解了${attackerName}的攻势。`,
    );
    return {
      action: createCombatAction(
        encounter,
  ...
  const effectiveCritRate = clamp(
    attacker.critRate +
      (skillAction
        ? (skill?.bonusCritRate ?? 0) * weaponAffinityEffectMultiplier
        : 0) -
      defender.critResist,
    0,
    100,
  ...
  const defensePierceRate = skillAction
    ? clamp(
        (skill?.defensePierceRate ?? 0) * weaponAffinityEffectMultiplier,
        0,
        100,
      )
    : 0;
  ...
  const defenseMultiplier =
    COMBAT_BALANCE.defenseDamageDenominator /
    (COMBAT_BALANCE.defenseDamageDenominator + effectiveDefense);
  const criticalMultiplier = critical
    ? 1 +
      ((attacker.critDamage - 100) / 100) *
        (1 - defender.critDamageReduction / 100)
    : 1;
  const roundDamageBonus =
    attackerRef.side === "player"
      ? activeRoundPassiveValue(encounter, "damage-bonus-for-rounds")
      : 0;
  const roundDamageReduction =
    defenderRef.side === "player"
      ? activeRoundPassiveValue(encounter, "damage-reduction-for-rounds")
      : 0;
  const effectiveDamageBonus = attacker.damageBonus + roundDamageBonus;
  const effectiveDamageReduction = clamp(
    defender.damageReduction + roundDamageReduction,
    0,
    100,
  );
  const damageMultiplier =
    (1 + effectiveDamageBonus / 100) *
    (1 - effectiveDamageReduction / 100) *
    (skillAction
  ...
  const baseDamage = Math.max(
    ...
    Math.round(
      attacker.attack *
        COMBAT_BALANCE.baseDamageMultiplier *
        defenseMultiplier *
        criticalMultiplier *
        damageMultiplier *
        randomFactor(random),
  ...
  encounter.logs.push(
    `${isCounter ? "反击：" : ""}${attackerName}${skillAction ? `以${skillAction.name}` : ""}命中，造成 ${baseDamage} 点伤害${critical ? "（暴击）" : ""}。`,
  );

  const roundComboBonus =
    attackerRef.side === "player"
      ? activeRoundPassiveValue(encounter, "combo-bonus-for-rounds")
      : 0;
  const comboRate = clamp(
    attacker.comboRate + roundComboBonus - defender.comboResist,
    0,
    100,
  );
  const comboRequested =
    !isCombo &&
    isAlive(encounter, defenderRef) &&
    canTrigger(comboRate, random);
  if (comboRequested)
    encounter.logs.push(`${attackerName}连击得手，准备再次攻击。`);

  const lifestealRate = clamp(
    attacker.lifestealRate - defender.lifestealResist,
    0,
    100,
  );
  if (lifestealRate > 0) {
    const healing = Math.round(
      totalDamage * (lifestealRate / 100) * (1 + attacker.healingBonus / 100),
    );
    if (healing > 0) {
  ...
  const skillStunRate = skillAction
    ? (skill?.stunRate ?? 0) * weaponAffinityEffectMultiplier
    : 0;
  if (
    isAlive(encounter, defenderRef) &&
    canTrigger(stunRate + skillStunRate, random)
  ) {
    defenderEffects.stunnedFor = Math.max(defenderEffects.stunnedFor, 1);
    encounter.logs.push(`${defenderName}陷入眩晕。`);
  }

  const counterRate = clamp(
    defender.counterRate - attacker.counterResist,
    0,
    100,
  );
  const counterRequested =
    ...
    !isCombo &&
    isAlive(encounter, defenderRef) &&
    canTrigger(counterRate, random);
  if (counterRequested)
    encounter.logs.push(`${defenderName}抓住空隙，发动反击。`);
  if (activeSkill?.grantDodge)
    attackerEffects.guaranteedDodge += Math.max(
      0,
      Math.floor(activeSkill.grantDodge),
    );
  return {
    action: createCombatAction(
      encounter,
...
function getTurnOrder(encounter: Encounter): EncounterTurnAction[] {
  const combatants: Array<{
    ...
    {
      combatant: { side: "player" },
      speed: encounter.playerStats.speed,
      tieBreaker: -1,
    },
    ...encounter.enemies
      ...
      .map(({ enemy, enemyIndex }) => ({
        combatant: { side: "enemy" as const, enemyIndex },
        speed: enemy.stats.speed,
        tieBreaker: enemyIndex,
      })),
  ];
  return combatants
    .sort(
      (left, right) =>
        right.speed - left.speed || left.tieBreaker - right.tieBreaker,
    )
    .map(({ combatant }) => toTurnAction(encounter, combatant));
}

function cloneForStep(
  encounter: Encounter,
  playerStats?: CombatStats | number,
  ...
  const next: Encounter = {
    ...
    playerOuterSkills: encounter.playerOuterSkills.map((skill) => ({
      ...skill,
    })),
  ...
  if (enemyStats && next.enemies[0]) {
    ...
    next.enemies[0].hp = Math.min(
      next.enemies[0].hp,
      next.enemies[0].maxHealth,
    );
  }
  return next;
}

function applyDungeonRoundStartEffect(encounter: Encounter): void {
  if (encounter.dungeonMechanic !== 'poison-mist' || encounter.round < 1) return
  const damage = Math.max(1, Math.round(encounter.playerMaxHealth * 0.012))
  applyDamage(encounter, { side: 'player' }, damage)
  encounter.logs.push(`幽篁毒雾侵入经脉，你受到 ${damage} 点持续伤害。`)
}
...
function selectPlayerSkill(
  ...
  isCounter: boolean,
  isCombo: boolean,
): MartialActiveSkill | undefined {
  if (
    attacker.side !== "player" ||
    isCounter ||
    isCombo ||
    encounter.playerEffects.stunnedFor > 0 ||
    encounter.playerRage < 100 ||
    !encounter.playerOuterSkills.length
  )
    return undefined;
  const index =
    encounter.nextOuterSkillIndex % encounter.playerOuterSkills.length;
  encounter.nextOuterSkillIndex =
    (index + 1) % encounter.playerOuterSkills.length;
  return encounter.playerOuterSkills[index];
}

function grantRageAfterAction(
  encounter: Encounter,
  attacker: CombatantRef,
  defender: CombatantRef,
  action: CombatAction,
  ...
  if (attacker.side === "player") {
    if (action.skill) {
      const refund = encounter.playerPassives.reduce((total, effect) => effect.kind === "skill-rage-refund" ? total + passiveValue(effect) : total, 0)
      encounter.playerRage += refund
    } else encounter.playerRage += 50
  }
  // A dodge is not a hit. Damage immunity still means the attack connected,
  // so it charges rage like a damaging hit. A stun is represented by the hit
  // that applies it; the later skipped turn must not charge rage again.
  if (
    defender.side === "player" &&
    (action.outcome === "hit" || action.outcome === "immune")
  )
    encounter.playerRage += 25;
}

export function advanceEncounterAction(
  encounter: Encounter,
  playerStats?: CombatStats | number,
  enemyStats?: CombatStats,
): EncounterStep {
  ...
  while (next.status === "fighting") {
    if (!next.actionQueue.length) {
      ...
      next.round += 1;
      applyDungeonRoundStartEffect(next);
      if (finishIfDefeated(next)) break;
    ...
    const resolution = performAttack(
      next,
      attackerRef,
      defenderRef,
      turnAction.isCounter,
      random,
      selectPlayerSkill(
        next,
        attackerRef,
        turnAction.isCounter,
        Boolean(turnAction.isCombo),
      ),
      Boolean(turnAction.isCombo),
    );
    grantRageAfterAction(next, attackerRef, defenderRef, resolution.action);
    const finished = finishIfDefeated(next);
    if (!finished && resolution.counterRequested)
      next.actionQueue.unshift(
        counterTurnAction(next, defenderRef, attackerRef),
      );
    if (!finished && resolution.comboRequested)
      next.actionQueue.unshift(comboTurnAction(next, attackerRef, defenderRef));
...
export function advanceEncounter(
  ...
  const targetRound = encounter.actionQueue.length
    ? encounter.round
    : encounter.round + 1;
  let next = encounter;
  do {
    ...
    (next.round < targetRound || next.actionQueue.length > 0)
  );
  return next;
}
